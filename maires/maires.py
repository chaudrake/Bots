...
def load_tweeted_maires():
    """Charge l'historique des tweets sans jamais le réinitialiser automatiquement"""
    tweeted = {}
    try:
        if os.path.exists(TWEETED_MAIRES_FILE):
            ...
            log_and_print("Fichier historique inexistant, il sera créé au premier tweet")
    except Exception as e:
        log_and_print(f"Erreur chargement historique: {e}")
    return tweeted

def save_tweeted_maire(code, name):
    """Sauvegarde un maire tweeté dans l'historique"""
    try:
...
def reset_tweeted_maires():
    """
    Réinitialise l'historique UNIQUEMENT quand tous les maires ont été tweetés.
    Cette fonction est appelée automatiquement par get_maire_to_tweet().
    """
    try:
        open(TWEETED_MAIRES_FILE, 'w').close()
        log_and_print("⚠️ RÉINITIALISATION DE L'HISTORIQUE - Tous les maires ont été tweetés")
    except Exception as e:
        log_and_print(f"Erreur réinitialisation: {e}")

def get_maire_to_tweet(df, tweeted_maires):
    """Trouve un maire à tweeter (jamais deux fois le même, sauf changement)"""
    today = pd.to_datetime('today').normalize()

    # Filtrer les mandats en cours
    ...
    # Garder le maire le plus récent par commune
    ...
    # Sélection aléatoire avec historique
    ...
    # Si on arrive ici, tous les maires actuels ont déjà été tweetés
    log_and_print("⚠️ Tous les maires actifs ont déjà été tweetés ! Réinitialisation de l'historique...")
    reset_tweeted_maires()
    # Après réinitialisation, on retourne un maire aléatoire
    row = df_active.sample(n=1).iloc[0]

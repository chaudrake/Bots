...
def generate_tweet_text(row):
    ...
    if circo:
        if sexe == "F":
            return f"🏛 La #députée de la {circo} {departement} est {prenom} {nom}{age_text} #Députés #AssembléeNationale 🇫🇷"
        else:
            return f"🏛 Le #député de la {circo} {departement} est {prenom} {nom}{age_text} #Députés #AssembléeNationale 🇫🇷"
    else:
        if sexe == "F":
            return f"🏛 La #députée {departement} est {prenom} {nom}{age_text} #Députés #AssembléeNationale 🇫🇷"
        else:
            return f"🏛 Le #député {departement} est {prenom} {nom}{age_text} #Députés #AssembléeNationale 🇫🇷"
...
def load_tweeted_deputes():
    ...
    try:
        if os.path.exists(TWEETED_DEPUTES_FILE):
            ...
            log_and_print(f"Historique chargé: {len(tweeted)} députés déjà tweetés")
        else:
...
def save_tweeted_depute(code, name):
    try:
...
def get_depute_to_tweet(df, tweeted_deputes):
    ...
    for _ in range(len(df_unique)):
        ...
        if code not in norm_tweeted:
            log_and_print(f"Sélectionné: {code} - {name} (nouveau député)")
            return row
...
def post_random_tweet(api):
    try:
        ...
        tweeted = load_tweeted_deputes()
        row = get_depute_to_tweet(df, tweeted)
        ...
        prenom = row["Prenom"]
        nom = row["Nom"]
        code_circo = str(row['Code de la circonscription législative']).zfill(4)
        save_tweeted_depute(code_circo, f"{prenom} {nom}")
...
def main():
    log_and_print("=== Démarrage Députés ===")
    log_and_print(f"WORKING_DIR: {WORKING_DIR}")
    log_and_print(f"Fichier historique: {TWEETED_DEPUTES_FILE}")
